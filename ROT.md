...
| R3 | **Staleness / expiry** | A time-boxed artifact (plan, status, priority list) outlives its validity and is reused as current | A 6-day-old execution plan was pasted into a fresh session and rebuilt yesterday's priorities | Weibull decay (runs on every scan) + battery `Expiry` (retrieved memory past its type's half-life) + `Freshness` (killed/decayed memories served) | **TESTED** |
...
| R5 | **Duplicate/echo memory** | The same fact stored N times crowds the context window and amplifies itself | Retrieval returned 2 identical trip-note memories for one query | Battery `Redundancy` + ingest novelty gate (ADD/NOOP/MERGE) + consolidation | **TESTED** |
