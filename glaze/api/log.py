...
Merges the system's real action trails into one newest-first feed:
  ...
  - superseded cubes: reconciliation batches — cubes a re-scan retired, grouped
    per source per day (glaze_cubes has no supersede timestamp, so the best
...
def _review_entries(conn, limit: int) -> list[dict]:
    rows = conn.execute(
        """SELECT r.decision, r.notes, r.reviewed_at, r.cube_id, c.title
           FROM reviews r LEFT JOIN glaze_cubes c ON c.id = r.cube_id
    ...
    try:
        rows = conn.execute(
            """SELECT t.cube_id, t.action, t.reason, t.rule_confidence,
               ...
               FROM triage_log t LEFT JOIN glaze_cubes c ON c.id = t.cube_id
...
def _superseded_entries(conn, limit: int) -> list[dict]:
    ...
    return [{
        ...
        "detail": (f"retired {r['n']} stale cube(s) from {r['source']} "
