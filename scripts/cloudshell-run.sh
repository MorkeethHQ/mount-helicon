...
#   1. Get the code:   git clone <your-public-repo-url> helicon && cd helicon
#   2. Provide the DB: upload data/helicon.db (Cloud Shell "Upload File"), or run a scan
...
if [ ! -f data/helicon.db ]; then
  echo "!! data/helicon.db missing. Upload it (Cloud Shell Upload File) or run: python3 -m helicon.cli scan"
  exit 1
fi
